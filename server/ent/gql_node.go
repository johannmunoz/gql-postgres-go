...
func (m *Manufacturer) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     m.ID,
		Type:   "Manufacturer",
		Fields: make([]*Field, 1),
...
func (pr *Product) Node(ctx context.Context) (node *Node, err error) {
	...
	if buf, err = json.Marshal(pr.Upc); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
	...
	err = pr.QueryManufacturer().
		Select(manufacturer.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	switch table {
	case manufacturer.Table:
		n, err := c.Manufacturer.Query().
			Where(manufacturer.ID(id)).
			CollectFields(ctx, "Manufacturer").
			Only(ctx)
		if err != nil {
			return nil, err
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	switch table {
	case manufacturer.Table:
		nodes, err := c.Manufacturer.Query().
			Where(manufacturer.IDIn(ids...)).
			CollectFields(ctx, "Manufacturer").
			All(ctx)
		if err != nil {
			return nil, err
