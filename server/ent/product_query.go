...
func (pq *ProductQuery) QueryManufacturer() *ManufacturerQuery {
...
func (pq *ProductQuery) WithManufacturer(opts ...func(*ManufacturerQuery)) *ProductQuery {
	query := &ManufacturerQuery{config: pq.config}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if query := pq.withManufacturer; query != nil {
		...
		for i := range nodes {
			if nodes[i].manufacturer_products == nil {
				continue
			}
			fk := *nodes[i].manufacturer_products
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "manufacturer_products" returned %v`, n.ID)
			}
			for i := range nodes {
