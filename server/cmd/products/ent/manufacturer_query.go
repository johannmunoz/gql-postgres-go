...
func (mq *ManufacturerQuery) QueryProducts() *ProductQuery {
...
func (mq *ManufacturerQuery) WithProducts(opts ...func(*ProductQuery)) *ManufacturerQuery {
	query := &ProductQuery{config: mq.config}
...
func (mq *ManufacturerQuery) sqlAll(ctx context.Context) ([]*Manufacturer, error) {
	...
	if query := mq.withProducts; query != nil {
		...
		nodeids := make(map[uuid.UUID]*Manufacturer)
		for i := range nodes {
		...
		query.Where(predicate.Product(func(s *sql.Selector) {
			s.Where(sql.InValues(manufacturer.ProductsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.manufacturer_products
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "manufacturer_products" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "manufacturer_products" returned %v for node %v`, *fk, n.ID)
			}
